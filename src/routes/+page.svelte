...
<style>
    ...
    @keyframes logoDrop {
        0%   { transform: translateY(-110vh) scale(1, 1);       opacity: 0; animation-timing-function: cubic-bezier(0.55, 0, 0.85, 0.2); }
        12%  { opacity: 1; }
        60%  { transform: translateY(0)      scale(1, 1);       opacity: 1; animation-timing-function: cubic-bezier(0.3, 0, 0.2, 1); }
        66%  { transform: translateY(0)      scale(1.18, 0.78); }                                      /* impact squash */
        74%  { transform: translateY(-12px)  scale(0.94, 1.08); animation-timing-function: cubic-bezier(0.5, 0, 0.6, 1); }  /* rebound up + stretch */
        82%  { transform: translateY(0)      scale(1.06, 0.94); }                                      /* second, smaller squash */
        90%  { transform: translateY(-3px)   scale(0.98, 1.02); }                                      /* tiny second rebound */
...
{#if pageState === "intro"}
    <div id="intro" bind:this={introEl} class:iris-out={irisOut}>
        ...
        <canvas
            id="crack-canvas"
            bind:this={crackCanvas}
            class:visible={showCracks}
        ></canvas>
    </div>
{/if}